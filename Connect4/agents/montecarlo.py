...
class MonteCarloAgent:
    ...
    def get_move(self, board):
        ...
        if board.PREV_MOVE is not None:
            self.currentNode = self.get_child_node(
                self.currentNode, board, board.PREV_MOVE, board.CURR_PLAYER
            )

        self.currentNode, col = self.montecarlo_tree_search(
            board, self.max_iterations, self.currentNode, self.timeout
        )
        self.currentNode = self.get_child_node(
            self.currentNode, board, col, board.PREV_PLAYER
        )
        return col
...
class Node:
    ...
    def selection(self):
        ...
        uct_val = lambda x: x.wins / x.visits + np.sqrt(
            2 * np.log(self.visits) / x.visits
        )
