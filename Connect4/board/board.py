...
class Board:
    ...
    def __str__(self):
        symbol_board = np.empty(self.board.shape, dtype=str)
        symbol_board[self.board == self.EMPTY] = "-"
        symbol_board[self.board == self.PLAYER1_PIECE] = "X"
        symbol_board[self.board == self.PLAYER2_PIECE] = "O"
        return str(np.flip(symbol_board, 0))
